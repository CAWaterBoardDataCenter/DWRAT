...
def PVP_main(data_range):
    ...
    for c, day in enumerate(data_range["Dates"].unique()):
        ...
        basin_rip_demand_data_T = np.dot(riparian_demand_data, riparian_user_connectivity_matrix_T)
    ...
    rip_upstream_allocations_matrix = np.dot(riparian_user_connectivity_matrix, rip_user_allocations_matrix)
...
###############################################################################################    
    ...
    pvp_flow_mod_matrix = np.dot(upstream_connectivity_matrix, pvp_flow_mod)
...
###############################################################################################
    ...
    basin_riparian_output.to_csv("output/basin_riparian_output" + out_file_name + ".csv")
    ...
    basin_appropriative_output.to_csv("output/basin_appropriative_output" + out_file_name + ".csv")
    ...
    return basin_appropriative_output, basin_riparian_output, user_output_df_app, user_output_df_rip
